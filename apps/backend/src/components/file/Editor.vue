...
const { createModel, sanitizeObject, getSchemaByName, generateFilename, renderDescription, getDateFromFilename } =
  useSchema()
...
const isModelChanged = computed(
  () => JSON.stringify(sanitizeObject(model.value)) !== JSON.stringify(sanitizeObject(initialModel.value))
)
...
const setEditor = async () => {
  ...
  if (serializedTypes.includes(mode.value) && schema.value?.fields) {
    ...
    if (['yaml', 'json', 'toml'].includes(mode.value) && schema.value.list) {
      schema.value.fields = [
        { name: 'listWrapper', type: 'object', list: true, label: false, fields: schema.value.fields }
      ]
...
const save = async () => {
  ...
  try {
    ...
    if (!sha.value) {
      const pattern = schema.value?.filename ? schema.value.filename : '{year}-{month}-{day}-{primary}.md'
      const filename = generateFilename(pattern, schema.value, model.value)
      currentPath.value = `${folder.value}/${filename}`
    }
    // Saving the file
    const saveData = await github.saveFile(
      props.owner,
      props.repo,
      props.branch,
      currentPath.value,
...
const checkConfig = debounce(async () => {
