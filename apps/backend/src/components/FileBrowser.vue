...
const goTo = (destination) => {
  ...
  if (targetDestination === undefined) {
    targetDestination = props.defaultPath || props.root
  }
  if (props.root && !targetDestination.startsWith(props.root)) {
    router.replace({ query: { ...route.query, 'fb-path': props.root } })
    console.warn(
      `You tried to access "${destination}" but the root is set to "${props.root}". Redirecting to "${props.root}".`
    )
