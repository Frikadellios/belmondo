...
const createRepo = async () => {
  ...
  if (!isValid) {
    notifications.notify(
      'The repository name is not valid; it must be alphanumeric with hyphens, underscores and periods.',
      'error',
      { delay: 10000 }
    )
    status.value = 'error'
  ...
  if (created) {
    notifications.notify(
      `"${selectedRepo.value}" successfully forked to "${owner.value.login}/${safeRepoName}".`,
      'success'
    )
