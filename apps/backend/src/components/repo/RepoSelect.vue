...
const isCurrentRepo = (item) => {
  return repoStore?.owner && repoStore.repo && item.full_name === `${repoStore.owner}/${repoStore.repo}`
}

const debouncedSearchRepos = debounce(async () => {
  if (query.value === '') {
    results.value = []
    return
  }
  ...
  if (searchResults) {
    ...
    results.value = searchResults.items.sort((a, b) => {
      const aPush = a.permissions?.push
      const bPush = b.permissions?.push
      return bPush - aPush
    })
    status.value = ''
  } else {
