...
const schemaFields = computed(() => {
  const fieldsArray = schema.value.fields
    ? JSON.parse(JSON.stringify(schema.value.fields))
...
const viewContents = computed(() => {
  ...
  if (view.search.trim()) {
    ...
    const limitedQuery = query.replace(/~(\d+)/g, (match, p1) => {
      ...
      const limitedEditDistance = Math.min(Number.parseInt(p1, 10), maxEditDistance)
      if (limitedEditDistance < Number.parseInt(p1, 10)) {
...
const setView = () => {
  ...
  view.config.fields =
    schema.value.view?.fields ||
    (collection.value[0]?.fields?.date ? [view.config.primary, 'date'] : [view.config.primary])
  view.config.sort =
    schema.value.view?.sort ||
    (collection.value[0]?.fields?.date ? ['date', view.config.primary] : [view.config.primary])
  view.sort = schema.value.view?.default?.sort || view.config.sort?.length ? view.config.sort[0] : null
  view.order = schema.value.view?.default?.order || 'desc'
...
const setCollection = async () => {
    ...
    .map((file) => {
      if (file.type === 'blob' && (extension.value === '' || file.name.endsWith(`.${extension.value}`))) {
        ...
        if (serializedTypes.includes(format.value) && schema.value?.fields) {
          try {
            contentObject = serialization.parse(file.object.text, {
              format: format.value,
              delimiters: schema.value.delimiters
            })
          } catch (error) {
        ...
        if (
          !contentObject.date &&
          (!schema.value.filename || schema.value.filename.startsWith('{year}-{month}-{day}'))
        ) {
  ...
  if (errorCount > 0) {
    ...
    notifications.notify(
      `Failed to parse frontmatter for ${errorCount} ${errorCount > 1 ? 'entries' : 'entry'}. Your settings may be wrong.`,
      'error',
      options
    )
  }
