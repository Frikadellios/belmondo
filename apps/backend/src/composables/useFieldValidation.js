...
export default function useFieldValidation() {
  ...
  const validateLength = (schema, value) => {
    ...
    if (schema.options?.minlength && value.length < schema.options.minlength) {
      errors.push(
        `This field must be at least ${schema.options.minlength} ${schema.options.minlength === 1 ? 'character' : 'characters'} long.`
      )
    }
    if (schema.options?.maxlength && value.length > schema.options.maxlength) {
      errors.push(
        `This field must be at most ${schema.options.maxlength} ${schema.options.maxlength === 1 ? 'character' : 'characters'} long.`
      )
    }
    return errors
  }
