...
const createConfigFile = async () => {
  ...
  if (data) {
    ...
    notifications.notify(`The configuration file (.pages.yml) couldn't be created. Try reloading the page.`, 'error', {
      delay: 0
    })
    status.value = ''
  }
...
const setRepo = async () => {
  ...
  if (!props.branch) {
    ...
    notifications.notify(
      `No branch provided. Redirecting you to the default branch ("${repoDetails.default_branch}").`,
      'warning'
    )
  ...
  if (repoDetails.default_branch !== props.branch) {
    if (repoBranchesNames.includes(props.branch)) {
      ...
      if (invalidBranch) {
        ...
        notifications.notify(
          `The branch "${props.branch}" doesn't exist. Redirecting you to the default branch ("${repoDetails.default_branch}").`,
          'error'
        )
  ...
  const otherBranches = repoBranchesNames.filter(
    (branch) => ![props.branch, repoDetails.default_branch].includes(branch)
  )
  ...
  if (!repoStore.config.object) {
    ...
    if (validationErrors.length > 0) {
      ...
      if (route.name !== 'settings') {
        notifications.notify(
          'Your settings are not valid, redirecting you to the settings page to fix errors.',
          'warning'
        )
        router.push({ name: 'settings' })
      } else {
...
watch([() => props.owner, () => props.repo, () => props.branch], async () => {
  await setRepo()
})
</script>