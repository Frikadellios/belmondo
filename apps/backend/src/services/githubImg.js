...
const getRawUrl = async (owner, repo, branch, path, isPrivate = false, decode = false) => {
  ...
  if (isPrivate) {
    ...
    if (
      !state.cache[parentFullPath]?.files?.[filename] ||
      Date.now() - (state.cache[parentFullPath]?.time || 0) > ttl
    ) {
...
const rawToRelativeUrls = (owner, repo, branch, html, encode = true) => {
  ...
  let newHtml = html // Use a new variable
  for (const match of matches) {
    const src = match[1] || match[2]
    const quote = match[1] ? '"' : "'"
    if (src.startsWith('https://raw.githubusercontent.com/')) {
      let relativePath = src.replace(
        new RegExp(`https://raw\\.githubusercontent\\.com/${owner}/${repo}/${branch}/`, 'gi'),
        ''
      )
      relativePath = relativePath.split('?')[0]
      ...
      newHtml = newHtml.replace(`src=${quote}${src}${quote}`, `src=${quote}${relativePath}${quote}`)
    }
  }

  return newHtml // Return the new variable
}
...
const swapPrefix = (path, from, to, relative = false) => {
  ...
  if (newPath?.startsWith('/') && relative) newPath = newPath.substring(1)
  ...
  if (html != null && from != null && to != null) {
    ...
    matches.forEach((match) => {
      ...
      if (from === to) {
        ...
        if (newSrc?.startsWith('/') && relative) newSrc = newSrc.substring(1)
