...
const CONFIG_CACHE = import.meta.env?.VITE_CONFIG_CACHE !== 'false'
...
const validate = (document, filterOneOf = false, filterIf = true) => {
  ...
  if (!valid && errors) {
    ...
    validation = errors.map((error) => {
      const yamlPath = error.instancePath
        ? error.instancePath
            .substring(1)
            .split('/')
            .map((segment) => (Number.isNaN(segment) ? segment : Number.parseInt(segment, 10)))
        : []
      ...
      if (error.keyword === 'additionalProperties') {
        ...
        const parentNode = yamlNode?.items?.find((item) => item.key.value === error.params.additionalProperty)
        ...
        range = yamlNode?.range ? yamlNode.range : null
