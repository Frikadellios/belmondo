  ...
  switch (error.response?.status) {
    ...
    case 403:
      notifications.notify(
        'Your do not have permissiopermissions are insuffici do not have permission to to  not allowed to perform this action.',
        'error'
      )
      break
  }
...
const saveFile = async (owner, repo, branch, path, content, sha = null, retryCreate = false) => {
  const attemptsMax = retryCreate ? 5 : 1 // Max attempts only apply if retryCreate is true
  let attempt = 0
